<?php
/**
* lokalizacja tego pliku: /components/com_k2/models/itemlist.php
* cel modyfikacji: sortujemy wyniki po pobraniu z bazy danych żeby ułożyć je w odpowiedniej chronologii
**/
...
class K2ModelItemlist extends K2Model
{
	...
	function getData($ordering = NULL)
	{
		...
		foreach($rows as $row){
			...
			$row->{'newArrayValue'} = $extraFieldsById[33];
			if($extraFieldsById[33] >= $dateNow){
				$futureItems[]= $row;
			} else if ($extraFieldsById[33] < $dateNow && $row->newArrayValue){
				$archiveItems[] = $row;
			} else {
				$otherItems[] = $row;
			}
		...
		usort($futureItems, function($a, $b){
			return strcmp($a->newArrayValue, $b->newArrayValue);
		});
		
		usort($archiveItems, function($a, $b){
			return strcmp($a->newArrayValue, $b->newArrayValue);
		});
		$archiveItems = array_reverse($archiveItems);
		$rows = array_merge($futureItems, $archiveItems);
		$rows = array_merge($rows, $otherItems);
		return $rows;
