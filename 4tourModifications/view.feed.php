<?php
/**
* adres pliku: /components/com_k2/views/itemlist/view.feed.php
* cel modyfikacji: dodajemy do feedu dodatkowe informacje spoza standartowego formatu
*/
/**
 * @version    2.7.x
 * @package    K2
...
$query->select('`extra_fields`, `plugins`');
...
$results = $db->loadRow(); 
$extraFields = json_decode($results[0], true);
$plugins = json_decode($results[1], true);
$fuckedUpEncoding = array("&oacute;", "oacute" );
$fields = array();
/* TO BE FIXED PROPERLY IN THE FUTURE
Łata żeby ominąć błędne wyświetlanie litery 'ó'
*/ 
foreach ($extraFields as $value) {
	$fields[$value['id']] = $value['value'];
}
	//extra lines end here
            $feedItem = new JFeedItem();
	    $feedItem->custom = "hardcoded value for now";
            $feedItem->title = $item->title;
            $feedItem->link = $item->link;
	//tutaj naprawiam kodowanie pól które nie są `extra_fields`
	 //   $fuckedUpEncoding = array("&oacute;", "oacute" );
	 //   $descriptionReplacement = str_replace($fuckedUpEncoding, "ó", $item->description);
         //   $feedItem->description = $descriptionReplacement;
		...
		$feedItem->itemId = $item->id;
		$feedItem->datawydarzenia = $fields[17];
		$feedItem->adreswydarzenia = $fields[3];
		$feedItem->wstep = $fields[22];
		$hourValue = $fields[21];
		$trueHour = 0;
		if(($hourValue %2) ==0) {
			$trueHour = (floor($hourValue / 2)-1) . ":30";
		} else {
			$trueHour = floor($hourValue / 2) . ":00";
		}

		$feedItem->godzina = $trueHour;
		
		//$feedItem->godzina = $fields[21];
		$feedItem->telefon = $fields[5];
		$feedItem->email = $fields[2];
		$feedItem->datazakonczenia = $fields[33];
		$feedItem->latitude = $plugins[latitude];
		$feedItem->longitude = $plugins[lognitude];
		$feedItem->itemImage = $item->itemImage;
		$feedItem->itemText = $item->itemText;
		$feedItem->bilety = $fields[35];
